...
{
    public class XUnitTemplate : ITestTemplate
    {
        public string GetTestAttributeName() => "[Fact]";
        public string GetTestClassAttributeName() => "";
        public string GetSetupAttributeName() => "// Constructor";
        public string GetTeardownAttributeName() => "// IDisposable.Dispose";
        ...
        public string GenerateTestClass(ClassAnalysis classAnalysis, GenerationOptions options)
        {
            ...
            // Test class
            sb.AppendLine($"    public class {classAnalysis.ClassName}Tests : IDisposable");
            sb.AppendLine("    {");
            
            // Fields
            sb.AppendLine($"        private readonly {classAnalysis.ClassName}? _sut;");
            ...
            foreach (var dependency in classAnalysis.Methods.SelectMany(m => m.Dependencies).Distinct())
            {
                if (dependency.StartsWith("I") && char.IsUpper(dependency[1]))
                {
                    if (options.MockingFramework == MockingFramework.Moq)
                    {
                        sb.AppendLine($"        private readonly Mock<{dependency}>? _mock{dependency.Substring(1)};");
                    }
                    else if (options.MockingFramework == MockingFramework.NSubstitute)
                    {
                        sb.AppendLine($"        private readonly {dependency}? _mock{dependency.Substring(1)};");
                    }
                }
            }
            ...
            // Constructor (Setup)
            sb.AppendLine($"        public {classAnalysis.ClassName}Tests()");
            sb.AppendLine("        {");
        ...
        public string GenerateTestMethod(MethodAnalysis methodAnalysis, string testCaseName, GenerationOptions options)
        {
            ...
            if (testCaseName.Contains("ShouldThrow"))
            {
                ...
                sb.AppendLine($"            Assert.Throws<{exceptionType}>(() => {{ /* TODO: Add test logic */ }});");
            }
            ...
            {
                sb.AppendLine("            Assert.True(true, \"TODO: Implement test assertions\");");
            }
